...
int main ()
{
  ...
  PID pid_steer = PID();
  pid_steer.Init(
    P_STEER_Kp, P_STEER_Ki, P_STEER_Kd, 
    P_STEER_MAX, P_STEER_MIN
  );
  ...
  pid_throttle.Init(
    P_THROTTLE_Kp, P_THROTTLE_Ki, P_THROTTLE_Kd, 
    P_THROTTLE_MAX, P_THROTTLE_MIN
  );
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          // =========================== Compute steer error (error_steer) from position & desired trajectory
          ...
          // =========================== Compute throttle error (error_throttle) from position & desired speed
